...
- **Standort-Konventionen (seit 07/2026, projektweit):** Legenden-Symbole sind überall abgerundete Quadrate (`icon: 'roundRect'`); Institute erscheinen in allen Charts, Tabellen und Exporten in der unter „Institute" konfigurierten Reihenfolge (`InstituteColor.sort_order`, serverseitig über `sortBranchIds()` in `SalesStatisticsService`) mit den zentral konfigurierten Farben (`BranchColorService`); der Sidebar-Standortfilter wirkt lückenlos — seit 07/2026 auch im „Standort-Vergleich seit Eröffnung" und im CSV-Export „Körperzonen pro Tag & Institut". Einschränkung: Legacy-Lastschrift-Aggregate (vor 06/2026) sind nicht standortscharf — bei gewähltem Standort zeigen MRR-/Rücklastschrift-Historie für Alt-Monate keine Werte. Verbindliche Regeln: `charts.instructions.md`, `statistics-pages.instructions.md`.
