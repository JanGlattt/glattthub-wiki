...
| `sessionExpired({ path })` (von `session-guard.js`, wenn die Hub-Sitzung im WebView abgelaufen ist): die App zeigt statt des Web-Modals „Sitzung abgelaufen" die **native Anmeldeseite** (Face ID läuft von selbst an) und lädt danach dieselbe Seite neu (`pendingURL`). Nur aus dem aktiven WebView; im Kiosk-Modus fehlt die Methode, dort bleibt das Modal (Befund Jan, 23.09.2026, Nr. 43; abgesichert durch `SessionGuardNativeHandoverTest`) | — |
...
| 1c | TestFlight-Pilot, Review, Custom-App-Einreichung, Miradore, Klickanleitungen | ✅ **TestFlight läuft** (22.09.2026): Build `1.0.0 (1)` gegen Prod, interne Gruppe „Pilot“ — Ablauf und Fallstricke in `.github/knowledge/ios-testflight-verteilung.md`; seit 24.09.2026 als Skript `ios/scripts/testflight-upload.sh ios|tvos` für beide Apps. Offen: zwei Testerinnen als ASC-Benutzer, ABM-Org-ID + Apps-und-Bücher-Token, Reviewer-Konto, Klickanleitungen Profil (App-Geräte) und Institut (Kiosk-Block) |
