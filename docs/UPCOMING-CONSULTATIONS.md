...
| **Buchungseingangsmatrix** | Heatmap Wochentag × 3-Stunden-Block über den **Buchungszeitpunkt** (nicht die Terminzeit) mit Σ-Zeilen/-Spalten; darunter Timeline (Buchungen pro Tag) mit Zoom-Regler zur freien Zeitraum-Wahl (Standard: letzte 6 Monate); Kennzahlen-Zeile rechnet mit | Dieselbe Matrix als Zahlen-Tabelle mit Summenzeile |
...
### Buchungseingangsmatrix (16.08.2026)

Neue Statistik-Karte `termine.booking-intake-matrix` (Asana „Neue Statistik - Buchungseingangsmatrix"): WANN Beratungstermine gebucht werden — Heatmap Wochentag (Zeilen) × 3-Stunden-Block (Spalten, 0–24 Uhr) über den **Buchungszeitpunkt**, für Erreichbarkeit, Kampagnen-Timing und Personalplanung.

- **Datenbasis `booking_trackings.booked_at`** (Entscheidung Jan, 16.08.2026): Das Buchungs-Tracking läuft seit 30.03.2026, **belastbar ab April 2026** (`BookingIntakeMatrixService::DATA_FROM = '2026-04-01'`) — frühere Zeiträume werden serverseitig abgeschnitten, das Info-Panel benennt die Grenze. Erfasst sind nur Online-Buchungen über das Widget; telefonisch/vor Ort angelegte Termine fehlen. Beratungs-Services über `consultation_services` (`is_active` + `is_consultation`), per `whereIn` statt Join (unterschiedliche Spalten-Kollationen der beiden Tabellen!).
- **Endpoint liefert Tageswerte, Matrix entsteht clientseitig:** `ReportController::bookingIntakeMatrix()` (Route `phorest.reports.booking-intake-matrix`, Gate `view_report_upcoming_consultations`) gibt je Kalendertag die 8 Block-Zähler zurück (lückenlose Tagesachse, 15 Min. Cache mit Zeitraum+Branch im Key). Die Karte aggregiert das Timeline-Fenster (ECharts-dataZoom unter der Matrix, Standard letzte 6 Monate) ohne Server-Roundtrip zur 7×8-Matrix; Kennzahlen-Zeile (Stat-Strip) rechnet aus demselben Fenster.
- **KPI-Quelle `booking-intake`** (KpiRegistry, shape `list`, `BookingIntakeMatrixService::getKpis()`): `total` (mit Vorperioden-Vergleich, nur wenn die Vorperiode vollständig in der belastbaren Historie liegt), `per_day`, `peak_slot`, `peak_weekday` — im Dashboard-Wizard wählbar, deklariert im Feld `kpis` der Statistik.
- **Dateien:** `App\Services\BookingIntakeMatrixService` (Aggregation, KPIs, `getWeekdayMatrix()` für den Export), Partial `resources/views/statistics/termine/booking-intake-matrix.blade.php`, JS `termine.booking-intake-matrix` in `public/js/statistics/termine.js`, Export-Quelle `booking-intake-matrix` in `ReportExportService` (Zeile je Wochentag×Block, exakt die UI-Zahlen). Tests: `tests/Feature/BookingIntakeMatrixTest.php`, `tests/Unit/BookingIntakeBlockTest.php`.
- **Gotchas:** viewKey (dataVersion **+ Timeline-Fenster**) steckt in allen `:key` der Heatmap-`x-for` (stale Item-Scopes); die Timeline nutzt die neue Theme-Klasse `.chart-canvas-glattt-xs` (160 px) — Höhen nie inline setzen.
