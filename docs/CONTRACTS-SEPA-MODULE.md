...
- **Endpoints:** `POST /hub/contracts/{contract}/payments/collect-sepa` (`ContractController::collectSepaPayment()`, Gate `manage_gocardless` + Rollen-Check) und `GET …/payments/collect-info` (`getSepaCollectInfo()` → `min_charge_date` aus `getCachedMandate()['next_possible_charge_date']`, Fallback `getNextPossibleChargeDate()`).
